...
class DbOperation
{
    ...
    function createHero($name, $realname, $rating, $teamaffiliation)
    {
        $stmt = $this->con->prepare(
            'INSERT INTO heroes (name, realname, rating, teamaffiliation) VALUES (?, ?, ?, ?)'
        );
    ...
    {
        $stmt = $this->con->prepare(
            'SELECT id, name, realname, rating, teamaffiliation FROM heroes'
        );
        $stmt->execute();
    ...
    {
        $stmt = $this->con->prepare(
            'UPDATE heroes SET name = ?, realname = ?, rating = ?, teamaffiliation = ? WHERE id = ?'
        );
        $stmt->bind_param(
            'ssisi',
            $name,
